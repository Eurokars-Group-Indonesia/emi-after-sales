...
return new class extends Migration
{
    ...
    {
        DB::unprepared("
            CREATE TRIGGER `tg_ms_user_roles_AFTER_UPDATE` AFTER UPDATE ON `ms_user_roles` FOR EACH ROW 
            BEGIN
                ...
                SET v_screen_id = IFNULL(SUBSTRING_INDEX(OLD.user_role_id, '-', 1), '');
                
                -- Generate Old Response using CONCAT
                SET v_old_response = CONCAT(
                    '{', CHAR(10),
                    '  \"user_id\": \"', IFNULL(OLD.user_id, ''), '\",', CHAR(10),
                    '  \"role_id\": \"', IFNULL(OLD.role_id, ''), '\",', CHAR(10),
                    '  \"assigned_date\": \"', IFNULL(OLD.assigned_date, ''), '\",', CHAR(10),
    ...
    {
        DB::unprepared("DROP TRIGGER IF EXISTS `tg_ms_user_roles_AFTER_UPDATE`");
    }
};
