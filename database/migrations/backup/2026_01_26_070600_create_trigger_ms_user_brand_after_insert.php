...
return new class extends Migration
{
    ...
    {
        DB::unprepared("
            CREATE TRIGGER `tg_ms_user_brand_AFTER_INSERT` AFTER INSERT ON `ms_user_brand` FOR EACH ROW 
            BEGIN
                DECLARE v_screen_id VARCHAR(50);
                DECLARE v_new_response LONGTEXT;
                
                -- Get Screen ID (Ensure it doesn't break if '-' is missing)
                SET v_screen_id = IFNULL(SUBSTRING_INDEX(NEW.user_brand_id, '-', 1), '');
    ...
    {
        DB::unprepared("DROP TRIGGER IF EXISTS `tg_ms_user_brand_AFTER_INSERT`");
    }
};
