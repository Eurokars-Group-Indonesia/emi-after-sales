...
return new class extends Migration
{
    ...
    {
        DB::unprepared("
            CREATE TRIGGER `tg_ms_dealers_AFTER_UPDATE` AFTER UPDATE ON `ms_dealers` FOR EACH ROW 
            BEGIN
                ...
                SET v_screen_id = IFNULL(SUBSTRING_INDEX(OLD.dealer_id, '-', 1), '');
                
                -- Generate Old Response using CONCAT
                SET v_old_response = CONCAT(
                    '{', CHAR(10),
                    '  \"dealer_code\": \"', IFNULL(OLD.dealer_code, ''), '\",', CHAR(10),
                    '  \"dealer_name\": \"', IFNULL(OLD.dealer_name, ''), '\",', CHAR(10),
                    '  \"city\": \"', IFNULL(OLD.city, ''), '\",', CHAR(10),
    ...
    {
        DB::unprepared("DROP TRIGGER IF EXISTS `tg_ms_dealers_AFTER_UPDATE`");
    }
};
