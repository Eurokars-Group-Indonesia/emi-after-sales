...
return new class extends Migration
{
    ...
    {
        DB::unprepared("
            CREATE TRIGGER `tg_ms_role_permissions_AFTER_UPDATE` AFTER UPDATE ON `ms_role_permissions` FOR EACH ROW 
            BEGIN
                ...
                SET v_screen_id = IFNULL(SUBSTRING_INDEX(OLD.role_permission_id, '-', 1), '');
                
                -- Generate Old Response using CONCAT
                SET v_old_response = CONCAT(
                    '{', CHAR(10),
                    '  \"role_id\": \"', IFNULL(OLD.role_id, ''), '\",', CHAR(10),
                    '  \"permission_id\": \"', IFNULL(OLD.permission_id, ''), '\",', CHAR(10),
                ...
                SET v_new_response = CONCAT(
                    ...
                    '  \"permission_id\": \"', IFNULL(NEW.permission_id, ''), '\",', CHAR(10),
    ...
    {
        DB::unprepared("DROP TRIGGER IF EXISTS `tg_ms_role_permissions_AFTER_UPDATE`");
    }
};
