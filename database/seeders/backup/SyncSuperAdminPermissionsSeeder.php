...
class SyncSuperAdminPermissionsSeeder extends Seeder
{
     ...
     * 
     * This seeder ensures SUPER ADMIN role has all permissions and menus
     * Run this after adding new permissions or menus to the system
     */
    public function run(): void
    {
        ...
        if (!$superAdminRole) {
            $this->command->error('SUPER ADMIN role not found. Please run migration first.');
            return;
        }
        
        $this->command->info('Syncing SUPER ADMIN permissions and menus...');
        
        // 1. Sync Permissions
        $permissions = DB::table('ms_permissions')->where('is_active', '1')->get();
        ...
        // 2. Sync Menus
        $menus = DB::table('ms_menus')->where('is_active', '1')->get();
        ...
        // 3. Sync Brands to SUPER ADMIN User
        ...
        if ($superAdminUser) {
            $brands = DB::table('ms_brand')->where('is_active', '1')->get();
        ...
        $this->command->info('');
        $this->command->info('========================================');
        $this->command->info('SUPER ADMIN SYNC COMPLETED');
        $this->command->info('========================================');
        $this->command->info("Total Permissions: " . count($permissions));
        $this->command->info("Total Menus: " . count($menus));
        if (isset($brands)) {
